...
1. A coding agent starts `waythrough serve` over stdio, as an MCP
   server.
2. `internal/cli` loads `~/.waythrough.yaml` and validates it through
...
`internal/config/presets.go` provides init-only presets for clojure-lsp,
gopls, typescript-language-server for JavaScript and TypeScript,
rust-analyzer, and pyright-langserver. These five entries form a fixed bound
for the interactive initializer. They are not runtime defaults. Each preset
...
Run `waythrough init` to select presets and create `~/.waythrough.yaml`, then
edit that file to customize startup, root markers, or any other language
server. The next `serve` start reads it as the complete configuration. See
the `language_servers` schema in `internal/config/config.go`.
